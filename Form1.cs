            ...
            for (k = 1; k <= n; k++)
            {
                ...
                if (checkBox16.Checked == true)
                {
                    ...
                    else
                    {
                        ...
                        d1 = Math.Round(random.NextDouble() * 1, 1);
                        d2 = random.Next(15, 75);
                        string s54 = "" + z + ")Вероятность того, что автомат при опускании одной монеты правильно сработает, равна 0,99. Найти наиболее вероятное число случаев неправильной работы автомата и вероятность этого числа, если будет опущено 200 монет. " + d2 + Environment.NewLine;
                        doc.InsertParagraph(s54);
                        double ans38, ans39;
                        ans38 = d1 * 0.01; ans39 = ans38 - 1;
                        string s55 = "" + "Задание " + z + "- " + ans39 + "<=k0<=" + ans38 + Environment.NewLine;
